...
session.headers.update(
    {'X-ApiKeys': f'accessKey={NESSUS_ACCESS_KEY}; secretKey={NESSUS_SECRET_KEY}', 'Content-Type': 'application/json'}
)
...
for scan_id, engagement_id in scan_engagement_mapping.items():
    ...
    if tests:
        tests.sort(
            key=lambda x: x.get('target_end') or "", reverse=True
        )
    ...
    if scan_date  <= last_scan_date:
        print(f'Scan {scan_id} ({scan_date}) is not newer than last uploaded scan ({last_scan_date}). Skipping...')
        continue
    ...
    with session.get(f"{NESSUS_URL}/scans/{scan_id}/export/{file_id}/download", stream=True) as r:
        r.raise_for_status()
